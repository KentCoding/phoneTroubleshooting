#Windows Solutions and Response Handlers
def solutions():
    solutions = ["screen", "battery", "speaker", "capacative", "touch button"] #Define array for given brand upon calling.
    return solutions
...
def issueHandler(issue, brand, query, ID): #Issue Handler function from response module
    ...
    if issue == "screen": #Check for issue to provide potential solution
        ...
        support.internalSupport(issue, "Have you tried plugging in your Windows phone?")
        ...
        support.internalSupport(issue, "Have you tried to reset your Windows phone to eliminate OS issues?")
        support.externalSupport(issue, True, brand, query, ID)
        return True
    elif issue == "capacative" or "touch button": #Example handler for multiple naming scenarios - see added array elements.
        support.internalSupport(issue, "Have you checked if your phone is reponsive? (i.e. boot up?)")
        support.internalSupport(issue, "Have you ensured that you are grounded when using the buttons?")
        support.externalSupport(issue, True, brand, query, ID)
        return True
